...
import chat.wewe.android.push.PushNotificationHandler;
...
import static chat.wewe.android.activity.MainActivity.waitingView;
...
abstract class AbstractAuthedActivity extends AbstractFragmentActivity {
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    methodCall = new MethodCallHelper(getApplication(), hostname);
  ...
  protected final void createRoom() {
    ...
    getMethodCallForSubmitAction().continueWith(task -> {
      if (task.isFaulted()) {
        Toast.makeText(getApplication(),  "Пользователя не найдено!", Toast.LENGTH_SHORT).show();
      } else {
        //  dismiss();
